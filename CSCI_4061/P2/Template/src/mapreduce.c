...
int main(int argc, char *argv[]) {
    ...
    key_t mapkey = ftok(".", MAPID);
  	...
  	msgctl(mid, IPC_RMID, 0); //close message queue for mapping processes 
    ...
    key_t redkey = ftok(".", REDID);
  	int mid2 = msgget(redkey, PERM | IPC_CREAT);
  	msgctl(mid2, IPC_RMID, 0); //close message queue for reducing processes
