...
# 3 "baseline_matvec_mult.c" 2
# 11 "baseline_matvec_mult.c"
...
int baseline_matrix_mult_vec(matrix_t mat, vector_t vec, vector_t res){
  ...
  for(int i=0; i<mat.rows; i++){
    ...
    for(int j=0; j<mat.cols; j++){
      int elij = ((mat).data[((i)*((mat).cols)) + (j)]);
      int vecj = ((vec).data[(j)]);
      int prod = elij * vecj;
      int curr = ((res).data[(i)]);
      int next = curr + prod;
