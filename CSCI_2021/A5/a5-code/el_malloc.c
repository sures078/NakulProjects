...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  if (block == el_ctl.heap_start) //check to see out of bounds
    return NULL;

  el_blockfoot_t *prev_foot =
    PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t));

  el_blockhead_t *prev_head =
    el_get_header(prev_foot);
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block){
  //linking after block is removed
  //previous and next being null accounted for
  if (block->prev == NULL)
...
el_blockhead_t *el_find_first_avail(size_t size){
  el_blockhead_t *start = el_ctl.avail->beg->next; //first block after dummy

  while(start != el_ctl.avail->end) { //iterating through blocks
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  if (block->size < new_size + EL_BLOCK_OVERHEAD) //insufficient size
    return NULL;

  size_t old_size = block->size;

  block->size = new_size; //initializes space of new_size

  el_blockfoot_t *new_foot = el_get_footer(block); //creates a new foot
  new_foot->size = new_size; //initializes space of new_size

  el_blockhead_t *new_head = el_block_above(block); //creates a new head
  new_head->size = old_size - new_size - EL_BLOCK_OVERHEAD; //remainder size
  el_blockfoot_t *new_foot_2 = el_get_footer(new_head); //creates second new foot
  new_foot_2->size = new_head->size; //remainder size
...
void *el_malloc(size_t nbytes){
  ...
  el_blockhead_t *head = el_find_first_avail(nbytes);
  if (head == NULL)
    return NULL;

  el_remove_block(el_ctl.avail, head); //removes head from available list

  el_blockhead_t *next_head = el_split_block(head, nbytes);

  //adds remaining space to available list
  if (next_head != NULL) {
    next_head->state = EL_AVAILABLE;
    el_add_block_front(el_ctl.avail, next_head);
  }

  head->state = EL_USED;
  el_add_block_front(el_ctl.used, head); //adds space to used list

  return PTR_PLUS_BYTES(head, sizeof(el_blockhead_t)); //pointer to space to be used
}
...
void el_merge_block_with_above(el_blockhead_t *lower){
  ...
  el_blockhead_t *higher = el_block_above(lower); //next head after lower

  //error checking
  if (higher == NULL || higher->state == EL_USED)
    return;

  el_remove_block(el_ctl.avail, lower); //unlinks lower from available list
  lower->size += higher->size + EL_BLOCK_OVERHEAD; //lower's new size

  el_blockfoot_t *lower_foot = el_get_footer(lower); //lower's new foot
  lower_foot->size = lower->size; //same size set

  el_remove_block(el_ctl.avail, higher); //unlinks higher from available list
  el_add_block_front(el_ctl.avail, lower); //adds combined space to front of list
}
...
void el_free(void *ptr){
  el_blockhead_t *head = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t)); //gets head
  ...
  el_merge_block_with_above(head); //merges head and next first

  el_blockhead_t *prev_head = el_block_below(head);
  el_merge_block_with_above(prev_head); //merges previous with head
}
