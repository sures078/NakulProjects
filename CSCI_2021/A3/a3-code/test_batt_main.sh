...
major_sep
printf "PROBLEM 1B test_batt_main.sh shell tests\n"

case "$1" in                    # either ./batt_main or ./hybrid_main
    ./batt_main)
        PROGRAM=$1
        shift
        ;;
...
if [ "$run_valg" = "1" ]; then
    ...
    for i in $all_tests; do
        ...
        $VALGRIND ${PROGRAM} ${input[i]} >& $VALGOUT
        ...
        then
            ...
            minor_sep
            echo "> $VALGRIND ./batt_main ${input[i]}"
            cat $VALGOUT
            minor_sep
        else
            printf "Valgrind OK\n"
