...
major_sep
printf "PROBLEM 3 hash_main.c\n"

generate=1
run_norm=1                                 # run normal tests
run_valg=1                                 # run valgrind tests
valg_denom=3                               # divide valgrind score by this for points
...
if [ "$run_valg" = "1" ]; then
    minor_sep
    printf "hash_main.c VALGRIND TESTS\n"

    for i in $all_tests; do
        printf "TEST %2d %-18s : " "$i" "${tnames[i]}"
        
        # run code through valgrind
        { printf "%s\n" "${input[i]}" | $VALGRIND ./hash_main -echo ; } >& $VALGOUT
